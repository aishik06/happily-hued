...
const SHOULD_USE_BASE_PATH = window.location.hostname.endsWith('github.io') || window.location.pathname.startsWith(`${BASE_PATH}/`) || window.location.pathname === BASE_PATH;

function withBasePath(path = '') {
  const normalized = path.startsWith('/') ? path : `/${path}`;
  const prefix = SHOULD_USE_BASE_PATH ? BASE_PATH : '';
